...
class WoocommerceSync {
  ...
  async syncOrder(orderId: string): Promise<void> {
    try {
      // Check if order exists in local system
      const existingOrder = await this.orderService.readByQuery({
        filter: {_and:[
          {sales_channel_order_id:{_eq: orderId}},
          {sales_channel:{_eq: this.salesChannel.id}}
        ]}
      });

      if (existingOrder && existingOrder.length > 0) {
        // Order exists - update status
        const wcOrder = await this.api.get(`orders/${orderId}`);
        await this.localOrderStatusUpdate(existingOrder[0].id, wcOrder.data.status);
      } else {
