...
export class OrderSync {
  ...
  async createOrder(orderId: string): Promise<void> {
    ...
    // Check if shipping address is available and complete, otherwise use billing address
    const useShippingAddress = orderInfo.data.shipping &&
      orderInfo.data.shipping.country &&
      orderInfo.data.shipping.city &&
      orderInfo.data.shipping.address_1 &&
      orderInfo.data.shipping.postcode;

    const shippingAddress: ShippingAddress = {
      country: useShippingAddress ? orderInfo.data.shipping.country : orderInfo.data.billing.country,
      city: useShippingAddress ? orderInfo.data.shipping.city : orderInfo.data.billing.city,
      address: useShippingAddress ? orderInfo.data.shipping.address_1 : orderInfo.data.billing.address_1,
      postcode: useShippingAddress ? orderInfo.data.shipping.postcode : orderInfo.data.billing.postcode,
    };
