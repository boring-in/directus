...
class StockCheck {
  constructor(database,services,schema) {
  ...
  async afterReplenishmentCheck(warehouseId) {
    let dataProvider = new DataProvider(this._database);
  ...
  async productSufficiencyCheck(orderData) {

    const { ItemsService } = this._services;
    let productMap = new Map();
 
    let orderService = new ItemsService("orders", { schema: this._schema });
